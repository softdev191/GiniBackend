	...
	if (body){
		...
		if (data) {
			data.forEach(function(obj){
				MemberTeam.findOrCreate({where: {member_id: obj.member_id , team_id: obj.team_id}, defaults: obj})
			  .then(([member, created]) => {
...
router.get('/get-member-team', function(req, res){
	let query= "SELECT m.member_id, m.team_id, t.team_name FROM member_teams_association m inner join teams t on m.team_id = t.team_id;";
...
router.get('/teams' , function(req, res){
	...
	Team.findAll({
	  ...
	  order: [
		...
		// 	    var keyA = new Date(a.jersey_number),
		// 	        keyB = new Date(b.jersey_number);
		// 	    // Compare the 2 dates
		...
		// 	});
		// });
		// // Adding delay to wait for loop to complete :(
		// delay(function(){
  //       console.log("World after 1 sec");
  //   }, 1000 );
